...
async function fetchFileSystem(path=fileSystemFilePath) {
  try {
...
function loadJSON(json) {
  ...
  // Verifica se json è una stringa JSON valida
  if (typeof json === 'string') {
    try {
      json = JSON.parse(json);
    } catch (e) {
      console.warn('Il parametro json non è una stringa JSON valida.');
      return null;
    }
  }

  // Verifica se json è un oggetto valido
  if (typeof json === 'object' && json !== null && !Array.isArray(json)) {
    console.log('JSON caricato con successo');
    return json;
  } else {
    console.warn('Il parametro json non è un oggetto valido.');
    return null;
  }
}

async function loadFileSystem(json = undefined) {
  let loadedJSON = loadJSON(json);

  if (loadedJSON === null) {
    loadedJSON = await fetchFileSystem();
  }

  if (loadedJSON) {
    fileSystem = loadedJSON;
    console.log('File system caricato con successo');
  } else {
    fileSystem = {
      '/': {
        'home': {
    ...
    console.log('Caricato file system di default');
  }
}
...
let position = "~";
let fileSystem = {};
let fileSystemFilePath = 'fileSystem.json'; // Percorso del file system JSON
